...
namespace mtt::daemon {
    ...
    auto Transform_Daemon::update() noexcept -> void {
        ...
        auto view = registry.view<ecs::Dirty_Mark<compo::Transform>>();
        for (auto entity: view) {
            if (registry.any_of<ecs::Dirty_Mark<math::Transform>>(entity)) continue;
            ...
            if (!dirty) dirty = up_trace(entity);
            if (dirty) down_trace(entity);
        }
    ...
    auto Transform_Daemon::up_trace(ecs::Entity entity) noexcept -> bool {
        ...
        if (transformed || !attached) return dirty || transformed;

        auto parent = hierarchy.parent(entity);
        dirty |= up_trace(parent);
        if (dirty) transform(entity);
        return dirty;
    }
    ...
    auto Transform_Daemon::down_trace(ecs::Entity entity) noexcept -> void {
        auto& hierarchy = *ecs::Hierarchy::instance;
        auto& registry = hierarchy.registry;
        if (!registry.any_of<compo::Transform>(entity)) return;

        transform(entity);
        for (auto child: hierarchy.children(entity)) down_trace(child);
    }
}
