...
namespace metatron::math {
	auto inline cartesion_to_unit_sphere(math::Vector<f32, 3> const& d) -> math::Vector<f32, 2> {
	...
	auto inline unit_sphere_to_cartesion(f32 cos_theta, f32 phi) -> math::Vector<f32, 3> {
	...
	auto inline unit_sphere_to_cartesion(math::Vector<f32, 2> const& s) -> math::Vector<f32, 3> {
		auto theta = s[0];
		auto phi = s[1];
	...
	auto inline sphere_to_cartesion(f32 r, f32 cos_theta, f32 phi) -> math::Vector<f32, 3> {
		auto c = unit_sphere_to_cartesion(cos_theta, phi);
		return r * c;
	}

	auto inline sphere_to_cartesion(math::Vector<f32, 3> const& s) -> math::Vector<f32, 3> {
		auto r = s[0];
		auto theta = s[1];
		auto phi = s[2];
