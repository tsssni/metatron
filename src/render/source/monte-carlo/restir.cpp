...
namespace mtt::monte_carlo {
    ...
    auto Restir_Integrator::Path::estimate(Estimation&& e) noexcept -> void {
        auto y = math::max(e.mis * e.beta * e.Li.value);
        ...
        r.shift(1.f, 1.f);
        if (this->r.merge(r, e.u)) {
            Li = e.beta * e.Li.value;
            gamma = e.beta;
            we = e.wi;
            mis = e.mis;
            pdf[2] = e.p_e;
            depth = (depth & half_mask) | (e.end << half_bits);
        }
    }
    ...
    near_field_distance(desc.near_field_distance) {}

    auto Restir_Integrator::upload(ref<Context> ctx) noexcept -> void {
        auto pixels = math::prod(uzv2{ctx.film->image.size});
        pathes[0] = pixels; pathes[0].alignment = sizeof(Path);
        pathes[1] = pixels; pathes[1].alignment = sizeof(Path);
    }
    ...
    auto Restir_Integrator::trace(ref<Context> ctx) noexcept -> void {
        ...
        auto trace = [&](auto&& px) {
            ...
            if (reuse_iterations == 0) {
                ...
                fixel = spectra::Stochastic_Spectrum{spec.lambda, p.Li};
        ...
        auto reuse = [&](auto&& px) {
            ...
            for (auto i = 0; i < spatial_samples; i++) {
                ...
                MTT_OPT_OR_CONTINUE(cs, replay(r, np, u[1]));
                auto& [c, cJ] = cs;
                ...
                MTT_OPT_OR_CONTINUE(ds, replay(nr, p, u[2]));
                auto& [d, dJ] = ds;

                if (false
                || !math::isfinite(cJ) || !math::isfinite(dJ)
                || !math::isfinite(d.r.p_hat)
                || !math::isfinite(c.Li)) continue;

                valid++;
                auto Mi = np.r.M;
                auto Mc = p.r.M;
                auto pni_hat = math::guarded_div(np.r.p_hat, cJ);
                auto pnc_hat = c.r.p_hat;
                auto mi = math::guarded_div(Mi * pni_hat, Mi * pni_hat + Mc / N * pnc_hat);
                auto pci_hat = d.r.p_hat * dJ;
            ...
            if (iter < reuse_iterations - 1) pathes[(iter + 1) & 0x1][pi] = dst;
                ...
                auto dz = iv2{dst.pixel & half_mask, dst.pixel >> half_bits};
                sp.ctx.pixel = dz;
                sp.ctx.dim = 4;
                sp.start();
                auto Li = spectra::Stochastic_Spectrum{sp.generate_1d()};
    ...
    auto Restir_Integrator::sample(ref<Ray> r) const noexcept -> opt<Path> {
        ...
        auto direct_lighting = [&]() {
            ...
            auto mis_u = 1.f / (mis_d + 1.f);
            acc_opt = r.accel(direct_ctx.r, direct_ctx.n);
            if (!acc_opt || !acc_opt->intr_opt) {
                path.estimate({{trace_ctx.lambda, l_intr.L}, gamma, mis_u, p_e, l_intr.wi, depth, rr, u[3]});
                return;
            }
        ...
        while (true) {
            ...
            if (!acc_opt || !acc_opt->intr_opt) {
                ...
                auto mis_e = specular ? 0.f : math::guarded_div(p_e, p);
                auto mis_w = 1.f / (1.f + mis_e);
                path.estimate({{trace_ctx.lambda, l_intr.L}, beta, mis_w, -p_e, path.wi, depth, rr, eu});
                break;
            }
            ...
            if (math::max(mat_intr.emission) > math::epsilon<f32>) {
                ...
                auto eu = r.sampler.generate_1d();
                auto mis_e = specular ? 0.f : math::guarded_div(intr.pdf * 1.f, p);
                auto mis_w = 1.f / (1.f + mis_e);
                path.estimate({{trace_ctx.lambda, mat_intr.emission}, beta, mis_w, -intr.pdf, path.wi, depth, rr, eu});
            }
            ...
            if (connectable && b_intr.connectable && intr.t > near_field_distance) {
                ...
                path.p = intr.p;
                path.wi = b_intr.wi;
                path.J = math::sqr(intr.t) / math::abs(math::unit_to_cos_theta(b_ctx.r.d));
                path.pdf[0] = p;
                path.pdf[1] = b_intr.pdf;
                path.depth = (path.depth & ~u32(half_mask)) | (depth - 1);
            }
        ...
        if (!math::isfinite(path.Li)) return {};
    ...
    auto Restir_Integrator::replay(ref<Ray> r, cref<Path> np, f32 u) const noexcept -> opt<std::tuple<Path, f32>> {
        auto p = np;
        auto J = 1.f;
        auto connection = p.depth & half_mask;
        auto end = p.depth >> half_bits;
        auto depth = 0u;
        auto beta = fv4{1.f};
        ...
        while (true) {
            ...
            auto delta = math::abs(p.mis) == 1.f;
            auto mised = connection + 2 == end;
            auto direct = mised && p.pdf[2] > 0.f;

            if (depth == connection) {
                auto rt = math::length(p.p - intr.p);
                if (rt < near_field_distance) break;
                auto wi = fv3{bt | fv4{(p.p - intr.p) / rt, 0.f}};
                ...
                auto wi = fv3{bt | fv4{(mised ? p.we : p.wi), 0.f}};
                MTT_OPT_OR_BREAK(c_intr, bsdf(b_ctx.r.d, wi, direct ? -1 : cu));
                b_intr = c_intr;
                b_intr.f *= math::abs(math::unit_to_cos_theta(b_intr.wi));

                auto pdf = direct ? p.pdf[2] : b_intr.pdf;
                auto div = direct ? 1 : b_intr.pdf / p.pdf[1];
                auto geo = math::sqr(intr.t) / math::abs(math::unit_to_cos_theta(b_ctx.r.d));
                p.pdf[1] = b_intr.pdf;

                auto mis_r = delta ? 0.f : !direct
                ? math::guarded_div(-p.pdf[2], b_intr.pdf)
                : math::guarded_div(b_intr.pdf, p.pdf[2]);
                p.mis = mised ? 1.f / (1.f + mis_r) : p.mis;

                beta *= b_intr.f / pdf;
                auto& base = mised ? p.gamma : p.beta;
                p.Li *= beta / base;
                base = beta;
                J *= div / geo;
                p.J = geo;
                p.r.p_hat = math::max(p.mis * p.Li);
                return std::tuple{p, J};
            } else {
