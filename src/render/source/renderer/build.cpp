...
namespace mtt::renderer {
    ...
    auto collect(
        ...
        auto& shapes = shape::Shape::vs::instance();
        for (auto i = 0; i < shapes.template size<T>(); ++i)
            if constexpr (std::is_same_v<T, shape::Mesh>) {
                auto mesh = shapes.template get<T>(i);
                primitives.push_back({
                    .type = opaque::Acceleration::Primitive::Type::mesh,
                    .mesh = shapes.template get<T>(i),
                });
            } else {
                ...
                for (auto j = 0; j < shapes.template get<T>(i)->size(); ++j)
                    prim.aabbs.push_back(shapes.template get<T>(i)->bounding_box(t, j));
                primitives.push_back(prim);
            }
        counts.push_back(counts.back() + shapes.template size<T>());
    }
    ...
    auto build(
        ...
        for (auto i = 0; i < dividers.size(); ++i) {
            auto& div = dividers[i];
            auto type = div.shape.idx.type();
            auto idx = div.shape.idx.index();
