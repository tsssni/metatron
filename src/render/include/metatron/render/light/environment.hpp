...
namespace metatron::light {
	struct Environment_Light final: Light {
		...
		auto operator()(math::Ray const& r, spectra::Stochastic_Spectrum const& Lo) const -> std::optional<std::unique_ptr<spectra::Stochastic_Spectrum>>;
		auto sample(eval::Context const& ctx, math::Vector<f32, 2> const& u) const -> std::optional<Interaction>;
	private:
		auto operator()(math::Vector<f32, 2> const& wi, spectra::Stochastic_Spectrum const& Lo) const -> std::optional<std::unique_ptr<spectra::Stochastic_Spectrum>>;
