...
namespace mtt::volume {
    ...
    template<typename T>
    auto from_nanovdb(cref<nanovdb::math::Vec3<T>> x) -> math::Vector<T, 3> { return {x[0], x[1], -x[2]}; }
    auto from_nanovdb(cref<nanovdb::math::Coord> x) -> iv3 { return {x.x(), x.y(), -x.z()}; }
    ...
    auto Nanovdb_Volume::to_local(cref<iv3> ijk) const noexcept -> fv3 {
        return from_nanovdb(grid()->indexToWorldF(to_nanovdb(fv3{ijk})));
    }
    auto Nanovdb_Volume::to_index(cref<fv3> pos) const noexcept -> iv3 {
        return from_nanovdb(grid()->worldToIndexF(to_nanovdb(pos)).floor());
    }
    ...
    auto Nanovdb_Volume::inside(cref<fv3> pos) const noexcept -> bool { return inside(to_index(pos)); }
    ...
    auto Nanovdb_Volume::operator()(cref<fv3> pos) const noexcept -> f32 {
        ...
        return Sampler(grid()->tree())(grid()->worldToIndexF(to_nanovdb(pos)));
    }
    ...
    auto Nanovdb_Volume::grid() const -> view<nanovdb::FloatGrid> { return handle->grid<f32>(); }
}
