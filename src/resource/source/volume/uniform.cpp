...
namespace mtt::volume {
    ...
    auto Uniform_Volume::to_index(cref<fv3> pos) const noexcept -> iv3 { return math::floor((pos - bbox.p_min) / voxel_size); }
    auto Uniform_Volume::dimensions() const noexcept -> uzv3 { return dims; }

    auto Uniform_Volume::inside(cref<iv3> pos) const noexcept -> bool { return math::all(
        [](i32 p, i32 q, auto){return p >= 0 && p < q;}, pos, dims
    );}
    auto Uniform_Volume::inside(cref<fv3> pos) const noexcept -> bool { return inside(to_index(pos)); }

    auto Uniform_Volume::bounding_box() const noexcept -> math::Bounding_Box { return bbox; }
    auto Uniform_Volume::bounding_box(cref<fv3> pos) const noexcept -> math::Bounding_Box { return bounding_box(to_index(pos)); }
